...
- Ingest seed corpus: `uv run ingest` (defaults to goethe; or `uv run ingest goethe`)
- Test all: `uv run pytest`
- Single test: `uv run pytest tests/test_scheduler.py::test_name -q`
- Lint/format: `uv run ruff check .` and `uv run ruff format .`

## Architecture
Code lives under `src/deutsch_haufig/`: `main.py` (FastAPI entry), `db.py`, `models.py` (Word, Sense, Example, Dialogue, User, ReviewCard, ReviewLog), `schemas.py`, `scheduler/` (FSRS behind a `Scheduler` interface), `ingest/` (`goethe.py`, `dwds.py`, `pipeline.py`), `dialogue/` (provider interface, on-demand LLM, cached in DB), `routes/` (`browse`, `word`, `learn`, `api`), `templates/` (Jinja2 + HTMX partials). Data: `data/goethe/` (cached CSVs), `data/dwds_cache/` (cached HTML + JSON). Ingest is idempotent on `(lemma, pos)`.
...
- Respect dwds ToS: single low-rate ingest, no redistribution of raw seed.
